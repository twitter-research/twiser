...
alphas = floats(min_value=0.01, max_value=1.0)
...
def gen_validate(results_list, true_value, alpha, test_null=True):
  ...
  pval_coverage = ss.binom_test(good_ci_count, n, alpha)
...
def test_ztest_stacked_fb_train_null(nobs1, nobs2, std1, std2, shift1, gu_params, alpha, seed):
  ...
  for _ in range(runs):
    ...
    fallback_cnt += fallback
    results_list.append(R)

  pval = gen_validate(results_list, true_value=0.0, alpha=alpha)
  print(pval)
  if fallback_cnt > 0:
    print(f"Warning fallback {fallback_cnt} / {runs}")
  else:
    assert pval >= PVAL_CUTOFF
